...
class OpenConnectorClient:
    ...
    def __init__(self, base_url: str = OPENCONNECTOR_URL, token: str = OPENCONNECTOR_RUNTIME_TOKEN):
            ...
            try:
                ...
                if sso_manager:
                    ...
                    if tokens and "openconnector" in tokens:
                        headers["Authorization"] = f"Bearer {tokens['openconnector']['access_token']}"
                        return headers
            except ImportError:
                pass
    ...
    async def execute_action(self, action: str, input_data: Dict[str, Any], user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.error(f"Open Connector API error: {e.response.status_code} - {e.response.text}")
                raise
...
class GmailIntegration:
    ...
    async def search_emails(self, query: str, max_results: int = 10, user_id: Optional[str] = None) -> List[Dict]:
        """Search Gmail threads by query."""
        result = await self.client.execute_action("gmail.search_threads", {
            "query": query,
            "maxResults": max_results
        }, user_id=user_id)
        return result.get("threads", [])
    
    async def get_recent_emails(self, days: int = 7, max_results: int = 20, user_id: Optional[str] = None) -> List[Dict]:
        """Get emails from the last N days."""
        query = f"newer_than:{days}d"
        return await self.search_emails(query, max_results, user_id=user_id)
    
    async def get_unread_emails(self, max_results: int = 20, user_id: Optional[str] = None) -> List[Dict]:
        """Get unread emails."""
        query = "is:unread"
        return await self.search_emails(query, max_results, user_id=user_id)
    ...
    async def get_message(self, message_id: str, user_id: Optional[str] = None) -> Dict:
        ...
        return await self.client.execute_action("gmail.get_message", {
            "messageId": message_id
        }, user_id=user_id)
    
    async def send_email(self, to: str, subject: str, body: str, is_html: bool = False, user_id: Optional[str] = None) -> Dict:
        """Send an email."""
        return await self.client.execute_action("gmail.send_email", {
            "to": to,
            "subject": subject,
            "body": body,
            "isHtml": is_html
    ...
    async def reply_to_email(self, message_id: str, thread_id: str, body: str, user_id: Optional[str] = None) -> Dict:
        """Reply to an email thread."""
        return await self.client.execute_action("gmail.reply_email", {
            "messageId": message_id,
            "threadId": thread_id,
            "body": body
        }, user_id=user_id)
    
    async def create_draft(self, to: str, subject: str, body: str, user_id: Optional[str] = None) -> Dict:
        """Create an email draft."""
        return await self.client.execute_action("gmail.create_draft", {
            "to": to,
            "subject": subject,
            "body": body
        }, user_id=user_id)
    ...
    async def get_profile(self, user_id: Optional[str] = None) -> Dict:
        ...
        return await self.client.execute_action("gmail.get_profile", {}, user_id=user_id)
    
    async def list_labels(self, user_id: Optional[str] = None) -> List[Dict]:
        """List all Gmail labels."""
        result = await self.client.execute_action("gmail.list_labels", {}, user_id=user_id)
        return result.get("labels", [])
    
    async def add_label_to_email(self, message_id: str, add_label_ids: List[str] = None, remove_label_ids: List[str] = None, user_id: Optional[str] = None) -> Dict:
        """Add/remove labels from an email."""
        input_data = {"messageId": message_id}
        if add_label_ids:
            input_data["addLabelIds"] = add_label_ids
        if remove_label_ids:
            input_data["removeLabelIds"] = remove_label_ids
        return await self.client.execute_action("gmail.add_label_to_email", input_data, user_id=user_id)
...
class OutlookIntegration:
    ...
    async def search_emails(self, query: str, max_results: int = 10, user_id: Optional[str] = None) -> List[Dict]:
        ...
        result = await self.client.execute_action("outlook.search_messages", {
            "query": query,
            "top": max_results
        }, user_id=user_id)
    ...
    async def send_email(self, to: str, subject: str, body: str, is_html: bool = False, user_id: Optional[str] = None) -> Dict:
        ...
        return await self.client.execute_action("outlook.send_mail", {
            "to": to,
            "subject": subject,
            "body": body,
            "isHtml": is_html
...
class EmailTriageSystem:
    ...
    def __init__(self, gmail: GmailIntegration, outlook: Optional[OutlookIntegration] = None):
        self.gmail = gmail
        self.outlook = outlook
    
    async def triage_recent_emails(self, days: int = 1, user_id: Optional[str] = None) -> Dict[str, List[Dict]]:
    ...
    async def generate_daily_digest(self, user_id: Optional[str] = None) -> Dict[str, Any]:
...
class MeetingIntelligence:
    ...
    async def get_meeting_prep(self, meeting_subject: str, meeting_time: datetime, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        related_emails = await self.gmail.search_emails(search_query, max_results=10, user_id=user_id)
...
class MorningBriefing:
    ...
    def __init__(self, email_triage: EmailTriageSystem, meeting_intel: MeetingIntelligence):
    ...
    async def generate_briefing(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if email_digest.get("urgent_count", 0) > 0:
            briefing["priority_items"].append(f"{email_digest['urgent_count']} urgent emails require attention")
        
        if email_digest.get("important_count", 0) > 0:
            briefing["priority_items"].append(f"{email_digest['important_count']} important emails to review")
...
class ReplyDraftingSystem:
    ...
    async def draft_reply(self, message_id: str, thread_id: str, context: str = "", user_id: Optional[str] = None) -> Dict[str, Any]:
...
Draft a concise, professional reply:"""
    ...
    async def draft_simple_reply(self, message_id: str, thread_id: str, tone: str = "professional", user_id: Optional[str] = None) -> Dict[str, Any]:
