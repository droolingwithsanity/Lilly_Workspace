...
cd /opt/openlive-bridge 2>/dev/null || mkdir -p /opt/openlive-bridge
BRIDGE_PORT=8788 \
BRIDGE_HOST=0.0.0.0 \
LILLY_AI_URL="http://127.0.0.1:8098" \
LILLY_SENSOR_URL="${SENSOR_SERVER_URL:-http://100.115.234.87:8099}" \
node src/index.js 2>/dev/null || echo "Bridge skipped (placeholder)" &
...
echo "  • Lilly AI        : :8098"
echo "  • YOLO Trainer    : :8199 (xced) — via pool config"
echo "==========================================="
echo ""

# ── Multi-server pool setup ───────────────────────────
# Load pool config for distributed training coordination
POOL_CONFIG="/home/xceb/Lilly_Workspace/lilly-app/core/lilly-ai/.training/pool_config.json"
if [ -f "$POOL_CONFIG" ]; then
    echo "Multi-server pool active:"
    python3 -c "
import json, sys
try:
    d=json.load(open('$POOL_CONFIG'))
    for k,v in d.get('servers',{}).items():
        role=v.get('role','worker')
        host=v.get('host','?')
        print(f'  • {k} ({role}): {host}:{v.get(\"port\",8098)}')
    print(f'  Strategy: {d.get(\"default_strategy\",\"hybrid\")}')
except: pass
" 2>/dev/null
fi
