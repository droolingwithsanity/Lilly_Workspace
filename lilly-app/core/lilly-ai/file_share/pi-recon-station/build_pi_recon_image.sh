...
if [[ -z "$BASE" ]]; then
  BASE="raspios-lite.img"
  echo "==> No base image given; will download Raspberry Pi OS Lite 64-bit"
  echo "    (set --base to control this)."
  BASE_URL="https://downloads.raspberrypi.com/raspios_lite_arm64/images/"
  BASE_URL_LATEST="$(curl -s "$BASE_URL" | grep -oE 'href="[0-9_]+/"' | tail -1 | tr -d '"' | sed 's#^#/#' )"
  if [[ -z "$BASE_URL_LATEST" ]]; then
    echo "!! Could not resolve latest image URL automatically." >&2
    echo "   Please pass --base <path-or-url> manually." >&2
    exit 2
  fi
  echo "    -> $BASE_URL_LATEST"
fi

WORK="$(mktemp -d /tmp/piimg.XXXXXX)"
IMG="$BASE"
if [[ "$IMG" == http* ]]; then
  echo "==> Downloading base image: $IMG"
  IMG="$WORK/base.img"
  curl -L --fail -o "$IMG" "$BASE"
fi
if [[ ! -f "$IMG" ]]; then
  echo "ERROR: base image not found: $IMG" >&2; exit 1
fi
