...
import httpx
from insomnia_provider import InsomniaProvider

WORKSPACE = Path(__file__).parent
SCRIPT = WORKSPACE / "training_manager.sh"

# Opaque Auth0 session id (server-side store); anything else is a stale blob
# left over from the old cookie-based store and must be cleared.
_A0_SESSION_RE = re.compile(r"^[A-Za-z0-9_-]{16,128}$")

VISION_SERVER = os.environ.get("VISION_SERVER_URL", "http://127.0.0.1:8198")
POOL_CONFIG = WORKSPACE / ".training" / "pool_config.json"

def _load_pool_config() -> dict:
    if POOL_CONFIG.exists():
        return json.loads(POOL_CONFIG.read_text())
    return {"servers": {}, "active_server": "xced", "default_strategy": "hybrid"}

def _save_pool_config(d: dict):
    POOL_CONFIG.parent.mkdir(parents=True, exist_ok=True)
    POOL_CONFIG.write_text(json.dumps(d, indent=2, default=str))

if os.environ.get("TRAINER_SERVER_URL"):
    TRAINER_SERVER = os.environ.get("TRAINER_SERVER_URL")
else:
    try:
        _pc = _load_pool_config()
        _xc = _pc.get("servers", {}).get("xced", {})
        if _xc.get("host") and _xc.get("trainer_port"):
            TRAINER_SERVER = f"http://{_xc['host']}:{_xc['trainer_port']}"
        else:
            TRAINER_SERVER = "http://127.0.0.1:8199"
    except Exception:
        TRAINER_SERVER = "http://127.0.0.1:8199"
...
async def _allowed_user(request: Request) -> Optional[dict]:
    ...
    if os.environ.get("ALLOW_LOCAL_TRAINING", "").lower() in ("1", "true", "yes"):
        return {"email": ALLOWED_EMAIL, "name": "Local Admin", "sub": "local"}
...
async def _run_shell(args, timeout: float = 30.0):
    """Async wrapper for _run_script for InsomniaProvider."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _run_script, args, timeout)

insomnia_provider = InsomniaProvider(run_shell=_run_shell)
...
async def training_page(request: Request):
    ...
    if not user:
        ...
        stale = request.cookies.get("_a0_session")
        if stale and not _A0_SESSION_RE.match(stale):
...
# ═══════════════════════════════════════════════════════════════════════
# Multi-server resource pool
# ═══════════════════════════════════════════════════════════════════════


async def _fetch_server_health(host: str, port: int) -> dict:
    """Check if a server is healthy via HTTP."""
    try:
        async with httpx.AsyncClient(timeout=4.0) as client:
            r = await client.get(f"http://{host}:{port}/health", timeout=4)
            if r.status_code == 200:
...
async def _fetch_server_port(host: str, port: int) -> dict:
    """Check if a specific port responds."""
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            r = await client.get(f"http://{host}:{port}/api/trainer/status", timeout=3)
            return {"open": True, "data": r.json()} if r.status_code == 200 else {"open": False}
    except Exception:
        return {"open": False}


def _discover_local_resources() -> dict:
    """Discover local server resources (CPU, RAM, GPU)."""
    import shutil
    import subprocess
    ...
    try:
        ...
        for line in out.strip().split("\n"):
            if "Mem:" in line:
                parts = line.split()
                ram_used_gb = round(int(parts[2]) / (1024**3), 1)
                break
    except Exception:
        pass
...
async def pool_status(request: Request):
    ...
    servers = {}
    local = _discover_local_resources()
    local["role"] = "self"
    local["host"] = "127.0.0.1"
    local["tailscale_ip"] = "100.108.122.95"
    servers["self"] = local

    for key, srv in pool.get("servers", {}).items():
        host = srv.get("host", "")
        port = srv.get("port", 8098)
        health = await _fetch_server_health(host, port)
        trainer = await _fetch_server_port(host, srv.get("trainer_port", 8199))
        servers[key] = {
            "host": host,
            "tailscale_name": srv.get("tailscale_name", key),
            "port": port,
            "trainer_port": srv.get("trainer_port", 8199),
            "role": srv.get("role", "worker"),
            "handles": srv.get("handles", []),
            "cpu": srv.get("cpu"),
            "ram_gb": srv.get("ram_gb"),
            "gpu": srv.get("gpu"),
            "notes": srv.get("notes", ""),
            "pooled": srv.get("pooled", True),
            "max_threads": srv.get("max_threads"),
            "max_ram_gb": srv.get("max_ram_gb"),
            "weight": srv.get("weight", 50),
            "tasks": srv.get("tasks", []),
            **health,
            "trainer": trainer,
        }
    ...
    return {
        "ok": True,
        "strategy": pool.get("default_strategy", "hybrid"),
        "servers": servers,
        "active_server": pool.get("active_server", "xced"),
    }


@router.post("/api/pool/config")
async def pool_config(request: Request):
    ...
    if "servers" in body and isinstance(body["servers"], dict):
        pool["servers"] = body["servers"]
    if "active_server" in body:
        pool["active_server"] = body["active_server"]
    if "default_strategy" in body and body["default_strategy"] in ("hybrid", "centralized", "distributed"):
        pool["default_strategy"] = body["default_strategy"]
    _save_pool_config(pool)
    return {"ok": True, "pool": pool}


@router.get("/api/pool/allocation")
async def pool_allocation(request: Request):
    """Get current resource allocation across servers."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    pool = _load_pool_config()
    prof = _load_resources()
    status = await pool_status(request)
    servers = status.get("servers", {})
    allocation = {}
    for key, srv in servers.items():
        if srv.get("pooled", True):
            tasks = srv.get("tasks", [])
            threads = srv.get("max_threads") or prof.get("threads", 4)
            ram = srv.get("max_ram_gb") or prof.get("ram_gb", 16)
            allocation[key] = {
                "threads": threads,
                "ram_gb": ram,
                "tasks": tasks,
...
@router.post("/api/pool/discover")
async def pool_discover(request: Request):
    """Discover resources on all pooled servers."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    pool = _load_pool_config()
    ...
    for key, srv in pool.get("servers", {}).items():
        host = srv.get("host", "")
        health = await _fetch_server_health(host, srv.get("port", 8098))
        trainer = await _fetch_server_port(host, srv.get("trainer_port", 8199))
        results[key] = {
            "host": host,
            "health": health,
...
def _training_chat_system() -> str:
    ...
    return (
        ...
        + ". IMPORTANT: Do not repeat or echo back observation descriptions or corrections the user has made. If the user corrects a description, accept the correction and do NOT repeat the original observation. Keep your responses fresh."
    )
...
@router.get("/api/training/automation/status")
async def automation_status(request: Request):
    """Get automation/presets status for training console."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    return insomnia_provider.status()

@router.post("/api/training/automation/run")
async def automation_run(request: Request):
    ...
    try:
        body = await request.json()
        result = insomnia_provider.run(body or {})
        return result
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)

@router.post("/api/training/automation/stop")
async def automation_stop(request: Request):
    ...
    try:
        body = await request.json()
        session_id = body.get("id", "") if body else ""
        return insomnia_provider.stop(session_id)
    except Exception:
        return insomnia_provider.stop("")

@router.post("/api/training/automation/save")
async def automation_save(request: Request):
    ...
    try:
        body = await request.json()
        from insomnia_runner import AutomationRule
        rule = AutomationRule(
        ...
        presets = insomnia_provider._load_presets()
        presets.append({"id": rule.id, "name": rule.name, "config": body})
        insomnia_provider._save_presets(presets)
        return {"ok": True, "id": rule.id, "name": rule.name, "saved": time.time()}
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)

@router.get("/api/training/automation/log")
async def automation_log(request: Request):
    """Fetch the live action log."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 200:
                data = r.json()
                return {"ok": True, "log": data.get("log", []), "showing": data.get("showing", False)}
    except Exception:
        pass
    return {"ok": True, "log": [], "showing": False}


# ═══ DM Conversation Training ═══════════════════════════════════════════════
# DM conversation training data paths (outside WORKSPACE)
_DM_WORKER_DIR = Path("/home/xceb/dm-worker")
PERSONAS_FILE = _DM_WORKER_DIR / "personas.json"
HANDLED_FILE = _DM_WORKER_DIR / "handled.json"
INTERACTIONS_FILE = _DM_WORKER_DIR / "interactions.json"
DM_TRAINING_SCRIPT = WORKSPACE / "trainer" / "dm_to_training.py"


def _load_dm_conversations() -> dict:
    """Load DM conversation data for training."""
    conversations = {"total": 0, "by_avatar": {}, "avatars": []}
    ...
    if INTERACTIONS_FILE.exists():
        try:
            ...
            conversations["total"] = len(items)
            conversations["by_avatar"] = by_avatar
            conversations["avatars"] = list(by_avatar.keys())
        except Exception:
            pass
    ...
    if HANDLED_FILE.exists():
        try:
            data = json.loads(HANDLED_FILE.read_text())
            seen = data.get("seen", [])
            messages = data.get("messages", {})
            conversations["handled_count"] = len(seen)
            conversations["detailed_count"] = len(messages)
            by_avatar: Dict[str, int] = {}
            for msg_id, msg_data in messages.items():
                avatar = msg_data.get("avatar", "")
                if avatar:
                    by_avatar[avatar] = by_avatar.get(avatar, 0) + 1
            existing = conversations.get("by_avatar", {})
            for avatar, count in by_avatar.items():
                existing[avatar] = existing.get(avatar, 0) + count
            conversations["by_avatar"] = existing
        except Exception:
            pass
...
@router.get("/api/training/dm-status")
async def dm_training_status(request: Request):
    """Get DM conversation training data status."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    status = _load_dm_conversations()
    return {
        "ok": True,
        "status": "dm_conversations",
        "interactions_available": INTERACTIONS_FILE.exists(),
        "handled_available": HANDLED_FILE.exists(),
        "conversations": status,
        "training_script": str(DM_TRAINING_SCRIPT),
    }


@router.post("/api/training/dm-generate")
async def dm_training_generate(request: Request):
    ...
    try:
        ...
        all_avatars = body.get("all", False)
        max_conv = body.get("max_conv", 50)

        if DM_TRAINING_SCRIPT.exists():
            cmd = [
                sys.executable, str(DM_TRAINING_SCRIPT),
                "--max-conv", str(max_conv),
            ]
            if all_avatars:
                cmd.append("--all")
            elif avatar:
            ...
            conversations = _load_dm_conversations()
            return {
                "ok": True,
                "generated": 0,
                "message": "Use dm_to_training.py for full generation",
...
@router.get("/api/training/dm-conversations")
async def dm_training_conversations(request: Request):
    """Get raw DM conversation data for training."""
    user = await _allowed_user(request)
    if not user:
        return _deny()
    status = _load_dm_conversations()
    return {
        ...
        "conversations": status,
        "format": "persona_lora",
        "markers": ["<|persona|>", "<|background|>", "<|style|>", "<|conversation|>", "<|user|>", "<|name|>"],
        "description": "Instagram DM conversations formatted for persona LoRA training",
    }
