...
class AvatarTrainingPipeline:
    ...
    def train_avatar(
        ...
        try:
            ...
            # Orchestrator review of this avatar's result (best-effort, never stops training)
            try:
                from trainer.orchestrator_review import review_persona_result

                review_persona_result(
                    avatar=avatar_key,
                    output_dir=str(output_dir),
                    config=config,
                    job="train_all_avatars",
                    llm=os.environ.get("TRAINING_LLM_REVIEW", "0") == "1",
                )
            except Exception as _rev_e:
                logger.warning(f"Orchestrator review skipped: {_rev_e}")
            return True
            ...
            # Orchestrator review of the failure (best-effort)
            try:
                from trainer.orchestrator_review import review_persona_result
    ...
    def train_all_avatars(self, parallel: bool = False) -> Dict[str, bool]:
        ...
        # Orchestrator plan review (best-effort)
        try:
            from trainer.orchestrator_review import review_persona_plan
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override training steps for all avatars",
    )
    parser.add_argument(
        "--gpu",
        action="store_true",
    ...
    if args.max_steps:
        for avatar_key in avatars_to_train:
            AVATAR_CONFIGS[avatar_key]["training_steps"] = args.max_steps
    ...
    # Orchestrator summary review (best-effort)
    try:
        from trainer.orchestrator_review import review_persona_summary

        review_persona_summary(
            results,
            job="train_all_avatars",
            llm=os.environ.get("TRAINING_LLM_REVIEW", "0") == "1",
        )
