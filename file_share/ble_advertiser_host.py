...
def _hci_cmd(hci: str, ogf_ocf_data: str) -> tuple:
    ...
    # Event 0x0e = Command Complete; last status byte 0x00 = success
    ok = rc == 0 and "0e" in out.lower()
    return ok, out + err
...
async def _adv_loop(name: str, service_uuid: str, web_ui_url: str) -> None:
    ...
    mode values (in _config["mode"]):
      ...
      "fast_pair"  — Google Fast Pair BLE (triggers Android popup) only
      "both" /
      "balanced"   — Classic + Fast Pair BLE simultaneously (best coverage)
      "all"        — same as "both"
    """
    global _advertising
    hci = _config["hci"]
    ...
    try:
        ...
        if do_ble:
            ...
            if not ok:
                logger.warning(f"[host-adv] LE set adv params: {out.strip()}")
            ...
            if not ok:
                logger.warning(f"[host-adv] LE set adv data: {out.strip()}")
            ...
            if not ok:
                logger.warning(f"[host-adv] LE set scan resp: {out.strip()}")

            # Enable LE advertising
            ok, out = _hci_cmd(hci, "0x08 0x000A 01")
            if not ok:
                logger.error(f"[host-adv] LE enable failed: {out.strip()}")
                if not do_classic:
                    _advertising = False
                    return
            else:
