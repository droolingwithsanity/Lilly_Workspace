...
AUTOPILOT_URL = os.environ.get(
    "AUTOPILOT_URL",
    "http://100.93.131.114:4000"
)
# The Lilly product ID (discovered via /api/products?workspace_id=default)
AUTOPILOT_PRODUCT_ID = os.environ.get(
    "AUTOPILOT_PRODUCT_ID",
    "ec8e611b-6f2a-4f9c-90f6-f7e3505bc180"
)
AUTOPILOT_WORKSPACE = os.environ.get(
    "AUTOPILOT_WORKSPACE",
    "default"
)
...
class LillyAutopilotClient:
    ...
    def __init__(self, workspace_id: str = None, product_id: str = None):
    ...
    async def get_product(self, product_id: str = None) -> Optional[dict]:
    ...
    async def list_ideas(self, product_id: str = None, limit: int = 10) -> list[dict]:
        """Get improvement ideas for a product."""
        pid = product_id or self.product_id
        try:
            client = await self._get_client()
            r = await client.get(self._url(f"products/{pid}/ideas"), params={"limit": limit})
            r.raise_for_status()
            return r.json()
        except Exception as e:
    ...
    async def get_pending_approvals(self, product_id: str = None) -> list[dict]:
    ...
    async def approve_idea(self, idea_id: str, product_id: str = None) -> bool:
        """Approve an idea."""
        pid = product_id or self.product_id
        try:
            client = await self._get_client()
    ...
    async def get_health_score(self, product_id: str = None) -> Optional[dict]:
    ...
    async def get_research_cycles(self, product_id: str = None) -> list[dict]:
    ...
    async def list_tasks(self, assignee: str = None) -> list[dict]:
...
def format_ideas_for_lilly(ideas: list[dict]) -> str:
    ...
    if not ideas:
        return "I don't have any new improvement ideas from my research system right now."
